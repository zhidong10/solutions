	...
	b.fn.jScrollPane = function(e) {
		function d(D, O) {
			...
			function o() {
				ap.height(t + "px");
				I = 0;
				X = ay.verticalGutter + ap.outerWidth(true);
				Y.width(aj - X - f);
				try {
			...
			function F() {
				...
				if (aE) {
					Y.width((al.outerWidth(true) - f) + "px")
				}
				Z = Y.outerHeight();
				q = Z / v;
				if (aE) {
			...
			function V(s, aI) {
				...
				if (aI) {
					...
					au.stop().animate({"top":s},400);
					ad(s)
					//scrollTop=-parseInt(s);
					//console.log("scrollTop插件b:"+scrollTop)
				}
			}
			function ad(aI) {
				if (aI === c) {
				...
				u(aL, aJ);
				Y.stop().animate({"top":s},600);
				scrollTops=-parseInt(s);
				//console.log("scrollTops插件:"+scrollTops)
					...
					if (aK.length && Y.find(aJ)) {
						if (al.scrollTop() === 0) {
							aI = setInterval(function() {
								if (al.scrollTop() > 0) {
									ab(aK, true);
									b(document).animate({"scrollTop":(al.position().top)},400);
									clearInterval(aI)
								}
							}, 50)
						} else {
